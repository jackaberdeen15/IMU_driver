...
bool LSM9DS1_Accel::begin(TwoWire &wire){
    ...
    writeRegister(CTRL_REG6_XL, 0b01100000); //119 Hz, +/- 2g

    return true;
}


void LSM9DS1_Accel::read(float &x, float &y, float &z){
    readRaw(x,y,z);
    x -= BiasX;
    y -= BiasY;
    z -= BiasZ;
...
void LSM9DS1_Accel::readRaw(float &x, float &y, float &z){
    uint8_t buffer[6];
    _wire->beginTransmission(I2C_ADDR);
    _wire->write(OUT_X_L_XL);
...
void LSM9DS1_Accel::calibrate(int samples){
    float sumX = 0, sumY = 0, sumZ = 0;

    Serial.println("Calibrating Accelerometer... Please keep the board still!");
...
uint8_t LSM9DS1_Accel::readRegister(uint8_t reg) {
...
void LSM9DS1_Accel::writeRegister(uint8_t reg, uint8_t value) {
